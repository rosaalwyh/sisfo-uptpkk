...
class ProfileController extends Controller
{
    ...
    public function index($id)
    {
        ...
        $data['data_user'] = DB::table('users')->where('user_id', $id)->get();
        return view('backend.profile.data_profile', $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        if (isset($_POST['submit'])) {
            if (isset($request->user_foto)) {
                Storage::delete([$request->user_foto]);
                if($request->hasFile('user_foto')){
                    $destination_path = 'public/user_foto';
                    $user_foto = $request->file('user_foto');
                    $user_foto_name = $user_foto->getClientOriginalName();
            ...
            return redirect('profile/'.$user->user_id)->with('success', 'Data berhasil diubah!');
        } else {
            $data['data_user'] = User::where('user_id', $request->user_id)->get();
            return view('backend.profile.update_profile', $data);
        }
    }
